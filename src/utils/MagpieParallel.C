...
void
allgatherStringBuffers(const Parallel::Communicator & comm, std::string & send_buffer, std::vector<std::string> & recv_buffers)
{
  // serial case
  if (comm.size() < 2)
  {
  ...
  libmesh_call_mpi
    (MPI_Allgatherv (reinterpret_cast<void *>(&send_buffer[0]), mysize, MPI_UNSIGNED_CHAR,
                     reinterpret_cast<void *>(&recv[0]), sendlengths.data(), displacements.data(),
                     MPI_UNSIGNED_CHAR, comm.get()));
