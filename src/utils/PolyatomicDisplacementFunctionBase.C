...
{
  // quick check if energy < min_energy
  if (energy < _energy_history[0])
    return 0;

  if (energy >= _energy_history.back())
    return nEnergySteps() - 1;

  // naive linear search; might have to fix if it's a performance drag
  for (unsigned l = 1; l < nEnergySteps(); ++l)
    if (energy >= _energy_history[l - 1] && energy < _energy_history[l])
      return l;
