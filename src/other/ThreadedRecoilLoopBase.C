    ...
    while (!recoils.empty())
    {
      ...
      if (recoil->_E > 0.0)
      {
            ...
            _rasterizer.isTrackedSpecies(recoil->_Z, recoil->_m))
        {
          ...
          {
            addNRTEntry(number_fractions);
            index = _pa_nrt.size() - 1;
          }
          ...
          if (species_index != std::numeric_limits<unsigned int>::max())
            for (unsigned int target_var = 0; target_var < _nvars; ++target_var)
            {
              ...
              _vacancy_buffer.push_back(MyTRIMDefectBufferItem(recoil->_pos, target_var, w));
              _interstitial_buffer.push_back(MyTRIMDefectBufferItem(recoil->_pos, target_var, w));
            }
          else
        ...
        else
        {
          ...
          if (recoil->_tag >= 0)
          {
            if (recoil->_state == MyTRIM_NS::IonBase::INTERSTITIAL)
              _interstitial_buffer.push_back(MyTRIMDefectBufferItem(recoil->_pos, recoil->_tag));
            else if (recoil->_state == MyTRIM_NS::IonBase::REPLACEMENT)
              addDefectToResult(
