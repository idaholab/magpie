...
InputParameters validParams<DefectAnnihilation>()
{
  ...
  params.addClassDescription("Anihilation rate kernel that is proportional to the product of the kernel variable and the coupled variable");
  params.addRequiredCoupledVar("v", "Other defect concentration");
  params.addParam<Real>("prefactor", 1.0, "Anihilation rate");
  return params;
}
